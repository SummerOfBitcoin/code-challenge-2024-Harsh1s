...
fn target_to_compact(target_hex: &str) -> u32 {
    let target_bytes = hex::decode(target_hex).expect("Failed to decode hexadecimal string");
...
pub fn mine_block() -> Result<()> {
    ...
    let (merkle_root, coinbase_tx, _, txids) = generate_roots(map.clone())?;
    ...
    let target_int =
        BigUint::from_str_radix(target, 16).expect("Failed to convert hex to integer in the block");
    ...
    loop {
        ...
        let block_hash_int = BigUint::from_str_radix(&block_hash, 16)
            .expect("Failed to convert block hash hex to integer");

        if block_hash_int <= target_int {
            println!("Success: Valid nonce discovered: {}", nonce);
    ...
    println!("Number of transaction IDs: {}", txids.len());

    writeln!(block_file, "Block Header: {}", mine_block)?;
    writeln!(block_file, "Coinbase Transaction: {}", coinbase_tx)?;

    for txid in txids {
        writeln!(block_file, "Transaction ID: {}", txid)?;
    }
